...
void kopiujUzytkownikow(vector <Uzytkownik> uzytkownicy)
{
    fstream plikTekstowy;
    plikTekstowy.open("kopiaUzytkownikow.txt", ios::out);
    //cout << "Plik nie istnieje!!!";
    ...
    {
        plikTekstowy << uzytkownicy[i].idUzytkownika << "|";
        plikTekstowy << uzytkownicy[i].login << "|";
...
string wprowadzenieHasla()
{
    ...
    if (haslo2 != haslo)
    {
        cout << "Podane hasla roznia sie od siebie!!! Powtorz operacje!!!" << endl;
        Sleep(1000);
        wprowadzenieHasla();
    }
    else
...
void zapisywanieWPlikuTymczasowymUzytkownikow(vector<Uzytkownik> &uzytkownicy,int idUzytkownika, string haslo)
{
    fstream plikTekstowy;
    plikTekstowy.open("kopiaUzytkownikow.txt", ios::out);
    int i = 0;
    while(i < uzytkownicy.size())
    {
        if(idUzytkownika == uzytkownicy[i].idUzytkownika)
        {
            plikTekstowy << uzytkownicy[i].idUzytkownika << "|";
            plikTekstowy << uzytkownicy[i].login << "|";
            plikTekstowy << haslo << "|" << endl;
        }
        else
        {
            plikTekstowy << uzytkownicy[i].idUzytkownika << "|";
...
void czyszczeniePlikuUzytkownikowPrzedPonownymZapisem(vector <Uzytkownik> &uzytkownicy)
{
    int i = 0;
    fstream plik;
    plik.open("kopiaUzytkownikow.txt", ios::out | ios::trunc);
...
void edytowanieHaslaUzytkownika(vector <Uzytkownik> uzytkownicy, int idUzytkownika)
{
    Uzytkownik dodanyUzytkownik;
    string haslo = "";
    kopiujUzytkownikow(uzytkownicy);

    for(vector<Uzytkownik>::iterator i = uzytkownicy.begin(); i < uzytkownicy.end(); i++)
    {
        if(i -> idUzytkownika == idUzytkownika)
        {
            dodanyUzytkownik.idUzytkownika = i -> idUzytkownika;
            dodanyUzytkownik.login = i -> login;
            dodanyUzytkownik.haslo = i -> haslo;
        }
    }
    system("cls");
    cout << ">>>EDYCJA HASLA UZYTKOWNIKA: " << dodanyUzytkownik.login << " <<<" << endl;
    cout << "Podaj stare haslo: " << endl;
    cin.sync();
    haslo = wczytajLinie();

    if (haslo == sprawdzanieHasla(uzytkownicy, dodanyUzytkownik.login, haslo))
    {
        dodanyUzytkownik.haslo = wprowadzenieHasla();
        haslo = dodanyUzytkownik.haslo;
        zapisywanieWPlikuTymczasowymUzytkownikow(uzytkownicy, idUzytkownika, haslo);
        remove ("Uzytkownicy.txt");
        rename ("kopiaUzytkownikow.txt", "Uzytkownicy.txt");
